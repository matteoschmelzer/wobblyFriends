...
### Transcript & Demographic data
We developed two Python scripts ([`fetch_metadata_demographics.py`](https://github.com/matteoschmelzer/wobblyFriends/blob/main/transcripts/Fetch%20scripts/fetch_metadata_demographics.py) and [`fetch_textgrid_transcripts.py`](https://github.com/matteoschmelzer/wobblyFriends/blob/main/transcripts/Fetch%20scripts/fetch_textgrid_transcripts.py) to scrape the transcript data from the [Balance Corpus Explorer](https://thebalancecorpus.warwick.ac.uk/explore#). This provided us with the [OpenAI Whisper](https://github.com/openai/whisper) transcripts that had been proof-checked by the authors. We decided to go with this approach instead of running Whisper locally because it saved time and provided us with higher quality transcripts for 10 dyads (only three dyads were provided in the [MDIG2026 Summer School GitHub repository](https://github.com/WimPouw/TilburgMultiscaleSummerschool2026)). This increase in data will give us more statistical power to test the hypotheses below. Note: the 10 dyads featured on the [Balance Corpus Explorer](https://thebalancecorpus.warwick.ac.uk/explore#) explicitly gave consent for their data to be shared publicly.

### Video & Audio data
Video and audio data from three dyads were made available to us through the [MDIG2026 Summer School GitHub repository](https://github.com/WimPouw/TilburgMultiscaleSummerschool2026). 


## Question 2 - Hypotheses

### **H1** Participants that are familiar with one another have a higher success rate. {.unnumbered}

<!--
[**add_lit_here**]
@pollmann2018, @boyle1994
-->

This is an exploratory result as we use the 10 dyads from the [Balance Corpus Explorer](https://thebalancecorpus.warwick.ac.uk/explore#) (*N*=400 trials) rather than the 4 dyads used in the confirmatory Hypothesis 1. For initial findings see Fig. [-@fig-relationshipFindings, a)]. We find there is no support for this hypothesis. Success rate was near-identical for both groups (friend 91.5%; stranger 91.0%; trial-level $\chi$²=0.00, p=1.00; pair-level Mann Whitney U=11.0, p=.831; logistic regression controlling for condition: OR=0.94 for stranger vs friend, p=.859). An overall ~91% success rate leaves little room for a relationship effect to surface, suggesting a likely ceiling effect, rather than a clean rejection of this hypothesis.


![**A)** Bar chart showing success rate for friend (*N*=200) and stranger (*N*=200) dyad trials. Error bars show 95% Wilson confidence intervals. **B)** Box plot showing time to success (seconds) at the trial level, by relationship. **C)** Box plot showing communicative effort, operationalised by clue-giver utterances before correct guess, at the trial level, by relationship.](transcripts/Figures/fig_relationship_findings.png){#fig-relationshipFindings}


### **H2** Dyads with participants that are familiar with one other will take longer to accomplish a trial (consisting of a round of taboo) in comparison to dyads that are unfamiliar with one another. We assume this because familiarity enables the use of shared humour and in-jokes, which may increase enjoyment but simultaneously slow down effective guessing. {.unnumbered}

<!--
[**add_lit_here**]
@pollmann2018, @boyle1994, @holmes2002
-->
...
### **H3** The use of gesture increases (in terms of number of gestures) when participants are familiar with one another, in comparison to when they are familiar with one another. {.unnumbered}

This is supported by @anderson2020, who finds that familiarity between interlocutors affects gesture rate.<!-- Further, @speer2024 find that while strangers converge, friends tend to diverge in face-to-face conversations. This is connecting to Van Der Wege et al.'s [-@vanderwege2021] findings, who find that friends overestimate their ability to communicate effectively.-->
...
### **H4** Participants on the balance board take longer to successfully complete the Taboo task, due to increased allotment of resources allocated to balancing. {.unnumbered}

This is based on studies that show that walking and unipedal standing causes participants to solve tasks more quickly, but we assume that balancing is more cognitively demanding as shown in another study.<!--add references-->
...
![**A)** Box plot showing time to success (seconds) at the trial level, by condition (board vs ground) (Mann Whitney U p=.292). **B)** Spaghetti plot showing the per-pair change in median time to success (seconds) from board to ground. Bold line shows the mean change across pairs (Wilcoxon signed-rank test p=.557).](transcripts/Figures/fig_condition_time_to_success.png){#fig-timeToSuccessFindings}
...
-   3 dyads à 40 trials (*N*=120) for confirmatory analyses, 10 dyads à 40 trials (*N*=400) for exploratory analyses of transcripts
...
We are working with secondary data, which was analysed under different research questions, and a subset was made publicly available by the authors of the original study (@li2026).
